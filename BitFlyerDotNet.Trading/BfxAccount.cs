...
namespace BitFlyerDotNet.Trading
{
    public partial class BfxAccount : IDisposable
    {
        ...
        BfxPositions _positions = new BfxPositions();
        ...
        public void Open()
        {
            ...
            _positions.Update(Client.GetPositions(BfProductCode.FXBTCJPY).GetContent());

            RealtimeSource.GetChildOrderEventsSource().Subscribe(coe =>
            {
                var productCode = _marketSymbols[coe.ProductCode];
                _markets[productCode].RedirectChildOrderEvents(coe);
                if (productCode == BfProductCode.FXBTCJPY && coe.EventType == BfOrderEventType.Execution)
                {
                    _positions.Update(coe).ForEach(e => PositionChanged?.Invoke(this, new BfxPositionChangedEventArgs(e)));
                }
            });
